...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	public int scan(String... basePackages) {
		...
		//启动扫描器扫描给定包
		//xiattong：扫描后的结果是什么呢？
		//答：扫描Bean,生成definitionHolder,最终还是调用了：BeanDefinitionReaderUtils.registerBeanDefinition(definitionHolder, this.registry)
		doScan(basePackages);
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			/**
			 * xiattong：for 代码块中的内容和 “given annotated classes” 模式类似，最主要的不同有两处：第 1 处
			 * 主要做了啥？
			 */
			...
			for (BeanDefinition candidate : candidates) {
				...
				//设置Bean的自动依赖注入装配属性等
				/**
				 * xiattong：for 代码块中的内容和 “given annotated classes” 模式类似，最主要的不同有两处：第 2 处
				 * 主要做了啥？
				 */
	 ...
	 * 对给定的 definition 应用更多设置，而不仅仅是扫描组件类所检索的内容。
